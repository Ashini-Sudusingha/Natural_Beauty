...
if (empty($fname)) {
    ...
    echo ("Please Select your Provinse");
} else if (empty($distric)){
    echo ("Please Select your Distric");
} else if (empty($city)){
    echo ("Please Select your city");
} else if (empty($gender)){
    echo ("Please Select your ");
} else{
    // Update the database
    Database::iud("UPDATE `user` SET `fname` ='".$fname."',`lname` ='".$lname."',`email` ='".$email."',
    `password` ='". $password ."',`username` ='". $username ."',`mobile` ='".$mobile."',`no` = '". $no ."', `line_1` ='".$line1."',`line_2` ='".$line2."',`province_p_id`='". $province ."',
    `distric_dis_id`='". $distric ."',`city_city_id`='". $city ."',`gender_gender_id`='". $gender ."'
     WHERE `id` = '".$user["id"]."'");
