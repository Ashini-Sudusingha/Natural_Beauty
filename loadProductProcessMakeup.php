...
$qm = "SELECT * FROM `stock` INNER JOIN `product` ON `stock`.`product_id` = `product`.`id` INNER JOIN `product_img` ON `product_img`.`product_id` = `product`.`id` 
WHERE `product`.`categroy_cat_id`='1' ORDER BY `stock`.`date` DESC";
$rsm = Database::search($qm);
$numm = $rsm->num_rows;
// echo($num);

$results_per_page = 8;
$num_of_pages = ceil($numm / $results_per_page);
// echo($num_of_pages);

$page_results = ($pageno - 1) * $results_per_page;
// echo($page_results);

$qm2 = $qm . " LIMIT $results_per_page OFFSET $page_results";
$rsm2 = Database::search($qm2);
$numm2 = $rsm2->num_rows;
// echo($num2);
...
class="col-span-1 duration-500 bg-white shadow-md rounded-xl hover:scale-105 hover:shadow-xl w-72 justify-items-center">
    ...
    <div class="col-span-1 duration-500 bg-white shadow-md w-72 rounded-xl hover:scale-105 hover:shadow-xl">
        ...
        <a href="singleProductView.php?s=<?php echo $d["stock_id"]; ?>"><img src="<?php echo $dm["proimage_id"]; ?>"
                alt="Product" class="object-cover h-80 w-72 rounded-t-xl"></a>


        <div class="px-4 py-3 w-72">
            <span class="mr-3 text-xs text-gray-400 uppercase"><?php echo $dm["titile"]; ?></span>
            <p class="block text-lg font-bold text-black capitalize truncate"><?php echo $dm["name"]; ?></p>
            <div class="flex items-center">
                <p class="my-3 text-lg font-semibold text-black cursor-auto">Rs.<?php echo $dm["customer_price"]; ?>
                    &nbsp;<lable class="text-sm"> Available:<?php echo $dm["qty"]; ?></lable>
                </p>
