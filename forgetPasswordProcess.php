...
if ($num > 0) {
    $vcode = uniqid();
    Database::iud("UPDATE `user` SET `v-code` = '" . $vcode . "' WHERE `email` = '" . $email . "'");
    ...
    try {
        ...
        $mail->Username = 'chanukarukshan81@gmail.com';                     //SMTP username
        $mail->Password = 'ousfzkbdsclgcfjj';                               //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

        //Recipients
        $mail->setFrom('chanukarukshan81@gmail.com','reset password');
        $mail->addAddress('chanukarukshan81@gmail.com','reset password');     //Add a recipient
